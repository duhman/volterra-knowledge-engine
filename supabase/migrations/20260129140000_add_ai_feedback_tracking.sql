...
CREATE TABLE IF NOT EXISTS volterra_kb.ai_response_feedback (
  ...
  channel_id TEXT NOT NULL DEFAULT 'YOUR_SLACK_CHANNEL_ID', -- #help-me-platform
  -- AI response details
...
BEGIN
  INSERT INTO volterra_kb.ai_response_feedback (
    ...
    p_thread_ts,
    COALESCE(p_channel_id, 'YOUR_SLACK_CHANNEL_ID'),
    p_ai_response_ts,
    p_ai_response_text,
    p_ai_confidence,
    p_ai_sources
...
BEGIN
  ...
  WHERE thread_ts = p_thread_ts
    AND channel_id = COALESCE(p_channel_id, 'YOUR_SLACK_CHANNEL_ID')
