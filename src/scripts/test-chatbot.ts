...
const TEST_QUESTIONS: TestQuestion[] = [
  {
    ...
    expectedTopics: ['norgespris', '40 øre', 'Elhub', 'styrepost@volterra.example.com'],
  },
  {
    category: 'App - Login',
  ...
  },
  {
    ...
    question: 'Jeg har solgt boligen min. Hvordan overforer jeg abonnementet til ny eier?',
  ...
  },
  {
    ...
    question: 'Ladekabelen sitter fast i ladestasjonen. Hvordan losner jeg den?',
  ...
  },
  {
    ...
    expectedTopics: ['91 59 05 00', '24/7', 'kontaktskjema'],
  },
];

async function testChatbot(): Promise<void> {
  const n8nBaseUrl = (process.env.N8N_API_URL || 'https://your-n8n-instance.example.com/api/v1').replace('/api/v1', '');
  ...
  const results: { category: string; passed: boolean; response: string; missingTopics: string[] }[] = [];
  ...
  for (const test of TEST_QUESTIONS) {
    ...
    try {
      ...
      const answerText = (data.output || data.text || data.response || JSON.stringify(data)).toLowerCase();

      const foundTopics = test.expectedTopics.filter(topic =>
        answerText.includes(topic.toLowerCase())
      );
      const missingTopics = test.expectedTopics.filter(topic =>
        !answerText.includes(topic.toLowerCase())
      );

      const passed = foundTopics.length >= Math.ceil(test.expectedTopics.length / 2);
      ...
      logger.info(`  Result: ${passed ? 'PASS' : 'FAIL'} (${foundTopics.length}/${test.expectedTopics.length} topics found)`);
      ...
      logger.error(`Test failed with error: ${error instanceof Error ? error.message : error}`);
      results.push({
        category: test.category,
        passed: false,
