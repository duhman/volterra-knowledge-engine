...
- Cite sources with clickable links:
  ...
  - Slack messages: https://your-workspace.slack.com/archives/{CHANNEL_ID}/p{MESSAGE_TS_NO_DOT}
    - Remove the dot from message_ts (e.g., 1765876995.242879 becomes p1765876995242879)
    - Channel #help-me-platform = YOUR_SLACK_CHANNEL_ID
...
async function updateWorkflow(): Promise<void> {
  ...
  if (workflow.settings?.errorWorkflow)
    cleanSettings.errorWorkflow = workflow.settings.errorWorkflow;
  if (workflow.settings?.timezone)
    cleanSettings.timezone = workflow.settings.timezone;
  if (workflow.settings?.executionTimeout)
    cleanSettings.executionTimeout = workflow.settings.executionTimeout;
  ...
  const toolConnections = Object.entries(updated.connections)
    .filter(([_, conn]) =>
      conn.ai_tool?.some((arr) => arr.some((c) => c.node === "AI Agent")),
    )
    .map(([name]) => name);
  ...
  logger.info(
    "  https://your-n8n-instance.example.com/workflow/iVcW0pyvfWPPQufj",
  );
