...
async function updateChatbotPrompt(): Promise<void> {
  ...
  for (const node of workflow.nodes) {
    if (node.type === "@n8n/n8n-nodes-langchain.agent") {
      ...
      (node.parameters.options as Record<string, unknown>).systemMessage =
        NEW_SYSTEM_PROMPT;
      agentUpdated = true;
    }
  ...
  logger.info(
    `- Tool description: ${vectorStoreUpdated ? "Updated" : "Not found"}`,
  );
}
...
updateChatbotPrompt()
  .then(() => process.exit(0))
  .catch((error) => {
    logger.error("Failed to update chatbot prompt", {
