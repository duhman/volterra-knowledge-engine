...
```json
{
  "p_channel_id": "YOUR_SLACK_CHANNEL_ID",
  "p_limit": 10
}
```
...
- Default channel: YOUR_SLACK_CHANNEL_ID (#help-me-platform)
- Max limit: 200
...
- #help-me-platform: YOUR_SLACK_CHANNEL_ID (default)
...
curl -X POST \
  ...
  -d '{"p_channel_id": "YOUR_SLACK_CHANNEL_ID", "p_limit": 10}'
```
...
curl -X POST \
  ...
  -H 'Authorization: Bearer YOUR_ANON_KEY' \
  -H 'Content-Type: application/json' \
  -d '{"p_thread_ts": "1768402473.696469", "p_channel_id": "YOUR_SLACK_CHANNEL_ID"}'
```
...
SELECT * FROM get_latest_slack_messages('YOUR_SLACK_CHANNEL_ID', 5);
...
SELECT * FROM get_slack_channel_summary('YOUR_SLACK_CHANNEL_ID', 7);
```
